...
- [`achievements.js`](./extension/js/achievements.js) — 成就清单（共 10 个），按四类分类：累计摸鱼时长、摸鱼天数、单日摸鱼、喝水次数
- [`achievement-engine.js`](./extension/js/achievement-engine.js) — 从摸鱼日志与喝水记录生成统计快照，计算进度并判断解锁，不直接读写存储
