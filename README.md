...
一个趣味 **Chrome 扩展**（Manifest V3）——每天给你生成专属「宜 / 忌」，记录摸鱼时长并按时薪折算入账，久坐到点还会在当前网页右下角温柔提醒你喝口水、起身活动。摸鱼时长会累积成「摸鱼等级」，还有一只常驻网页的悬浮桌宠陪你摸鱼掉币，可在完整页一键切换腾讯企鹅 / 京东 Joy / 美团袋鼠 / 瑞幸梅花鹿 / 百度小熊等大厂吉祥物，抚摸还会冒出带大厂彩蛋的文字气泡。摸鱼与喝水都能解锁趣味成就，边摸边收集。
...
- **悬浮桌宠**：一只常驻任意网页右下角的桌宠，可拖动、可收起；在完整页可一键切换 6 位搭子（摸鱼小猫、腾讯企鹅、京东 Joy、美团袋鼠、瑞幸梅花鹿、百度小熊）；抚摸有 50% 概率掉 1~3 枚摸鱼币、50% 概率冒出与对应大厂强关联的彩蛋文字（每日 10 次上限），闲时还会冒出「劝摸」气泡
...
│   │   ├── pet-registry.js       # 桌宠图鉴（6 位搭子的造型 SVG + 文案/大厂彩蛋文字，content script 与完整页共用）
│   │   ├── pet-content.js        # 悬浮桌宠内容脚本（抚摸掉币 + 劝摸气泡 + 彩蛋文字，注入任意网页）
...
[`pet-content.js`](./extension/js/pet-content.js) 作为内容脚本注入任意网页右下角，自包含（内嵌样式 + 交互）；6 位搭子的造型 SVG 与文案统一登记在 [`pet-registry.js`](./extension/js/pet-registry.js) 的 `MoyuPets` 图鉴中，由完整页「桌宠搭子」面板写入 `pet-selected` 实现跨页面实时切换。数据存 `chrome.storage.local`：

- **多角色切换**：摸鱼小猫（默认）、腾讯企鹅、京东 Joy、美团袋鼠、瑞幸梅花鹿、百度小熊，每个角色有独立的登场问候、待机气泡、抚摸回应
- **大厂彩蛋文字**：抚摸时 50% 概率冒出与对应大厂强关联的彩蛋气泡（如「滴滴滴！你的摸鱼等级已升到皇冠👑」「美团外卖，送啥都快——摸鱼也不例外🛵」）
- **抚摸掉币**：点按桌宠，50% 概率掉 1~3 枚摸鱼币，每日 10 次上限
...
| `pet-selected` | 当前选中的桌宠 id（`cat` / `penguin` / `joy` / `kangaroo` / `deer` / `bear`） |
