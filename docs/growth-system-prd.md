...
|---|---|---|
| Lv.1 | 初级摸鱼学徒 | 刚学会假装在忙 |
| Lv.2 | 带薪如厕研究员 | 厕所是第一生产力 |
| Lv.3 | 划水中级工程师 | 摸鱼开始有技术含量 |
| Lv.4 | 资深咸鱼 | 躺平已成肌肉记忆 |
| Lv.5 | 摸鱼特级技师 | 老板走过都看不出来 |
| Lv.6 | 划水艺术家 | 摸鱼是一种美学 |
| Lv.7 | 摸鱼界扫地僧 | 深藏功与名 |
| Lv.8 | 摸鱼之神（封顶） | 传说级，已羽化登仙 |

### 5.2 升级时长曲线 🔴（关键数值，请重点确认）

> 🔴 **以下时长曲线直接决定养成节奏，是最需要你拍板的数值。** 采用"前松后紧"的缓增长：前期让用户快速尝到升级甜头，后期拉长以保留长期目标感。累计时长指历史总摸鱼时长。

| 段位 | 达成累计摸鱼时长（建议值 🔴） | 距上一级增量 |
|---|---|---|
| Lv.1 | 0（默认起点） | —— |
| Lv.2 | 2 小时 | +2h |
| Lv.3 | 6 小时 | +4h |
...
> 🔴 **待确认点**：① 段数是否为 8；② 封顶时长 240h 是否合理（按日均 30min 摸鱼约需 1.3 年封顶）；③ 是否需要封顶后的"荣誉小数点等级"（如 Lv.8.1）延长长线。
